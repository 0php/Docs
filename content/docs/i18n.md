...
locales();                 // ['en', 'id', 'it', 'cn'] (configured for web)
locales('mail');           // ['en', 'id'] (configured for mail)
```
...
Mail::send(function ($mail) use ($user, $token) {
    // compose the password-reset email — it renders in the 'mail' locale
});
...
```php
Translator::availableLocales();        // ['en', 'id', 'it', 'cn']
